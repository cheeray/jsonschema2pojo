...
public class OneOfRule implements Rule<JPackage, JType> {
	...
	private JFieldVar addEnumFields(JDefinedClass _enum) {
		...
		JVar jsonProp = forEach.body().decl(
				_enum.owner().ref(JsonProperty.class), "jsonProp");
		jsonProp.init(var.invoke("getAnnotation").arg(
				_enum.owner().ref(JsonProperty.class).dotclass()));

		JConditional ifJsonProp = forEach.body()
				._if(jsonProp.ne(JExpr._null()));

		// Find field name ...
		JVar fn = ifJsonProp._then()
				.decl(_enum.owner().ref(String.class), "fn");
		fn.init(jsonProp.invoke("value"));

		JConditional ifNotNamed = ifJsonProp._then()._if(fn.eq(JExpr._null()));
		ifNotNamed._then().add(
				(JStatement) JExpr.assign(fn, var.invoke("getName")));

		JConditional ifRequired = ifJsonProp._then()._if(
				jsonProp.invoke("required"));
		...
		matchRequires._else().add(
				parent.owner()
						...
						.arg(parent
								...
								.ref(String.class)
								.staticInvoke("format")
								.arg("Missing required: %s")
								.arg(parent.owner().ref(Arrays.class)
										.staticInvoke("toString")
										.arg(requires.invoke("toArray")))));
